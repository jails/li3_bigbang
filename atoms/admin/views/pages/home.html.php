...
$notify = function($status, $message, $solution = null) {
	...
	$html .= "<div class=\"test-result\">{$solution}</div>";
	return $html;
};
...
$support = function($classes) {
	...
	foreach ($classes as $class => $enabled) {
		...
		$item = "<span class=\"indicator{$enabled}\">{$status}</span>";
...
$checks = array(
	...
	'tests' => function() use ($notify, $self) {
		$tests = $self->html->link('run all tests', '/test/all', ['scope' => false]);
		$dashboard = $self->html->link('test dashboard', '/test', ['scope' => false]);
